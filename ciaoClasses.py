...
class Swift(Telescope):
    ...
    def filter_energy(self, e_range, *args, newfile="filtered_energy.fits"):
        """Energy ranges should be input as tuples. For example, a range from 100-400ev would be represented as (100,400), and ranges from 100-400eV, 500-900ev, and 1000-2000eV would be represented as (100,400), (500,900), (1000,2000).
        """
        ranges = f"{e_range[0]}:{e_range[1]}"
        for e_rng in args:
            try:
    ...
    def find_centroids(self):
        """This method works if only a single significant source is present. It yields an incorrect/misleading value if the signal-to-noise ratio of the image is low or there are multiple sources.
        """
        # make "Data Products" directory
        try:
            os.mkdir(f"{self.file_dir}/DataProducts")
        ...
        with fits.open(f"{self.file_path}") as file:
            source_x = np.array(file[1].data["X"])
            source_y = np.array(file[1].data["Y"])

            length = source_x.shape
            sum_x = np.sum(source_x)
            sum_y = np.sum(source_y)
            centroid_x, centroid_y = sum_x/length, sum_y/length
            return centroid_x[0], centroid_y[0]

    def sweeping_square(self, num_cells):
        with fits.open(f"{self.file_path}") as file:
            xs = np.array(file[1].data["X"])
            ys = np.array(file[1].data["Y"])

            X_CELLS = Y_CELLS = int(np.round(np.sqrt(num_cells)))

            WIDTH = 2 * int(np.amax(xs) - np.amin(xs))
            HEIGHT = 2 * int(np.amax(ys) - np.amin(ys))
            CELL_WIDTH, CELL_HEIGHT = WIDTH//X_CELLS, HEIGHT//Y_CELLS
            ...
            for x in range(0, WIDTH, CELL_WIDTH):
                for y in range(0, HEIGHT, CELL_HEIGHT):
                    cell_minx, cell_maxx = x-CELL_WIDTH, x
                    cell_miny, cell_maxy = y-CELL_HEIGHT, y

                    cell_constraints = np.where((xs < cell_maxx) & (
                        xs >= cell_minx) & (ys < cell_maxy) & (ys >= cell_miny))

                    cell_xs = xs[cell_constraints]
                    cell_ys = ys[cell_constraints]
                    coord_pairs = list(zip(cell_xs, cell_ys))

                    cell_count = len(coord_pairs)
                    image_count += cell_count

                    coord_counts = Counter(coord_pairs).most_common(3)
                    if len(coord_counts) >= 1:
                        most_common_loc, highest_count = coord_counts[0][0], coord_counts[0][1]
                        for result in coord_counts:
                            image_maxes.append(result)

                    # try:
                    #     locations, counts = np.unique(
                    #         coord_pairs, axis=0, return_counts=True)
                    #     m = counts.mean()
                    #     sd = counts.std()
                    #     snr = sd/m
                    # except ValueError:
            ...
            MASTER = np.array(list(zip(xs, ys))).shape[0]
            print(
                f"Image maximum: {highest_count}; location: {most_common_loc}")
            print(f"true event count: {MASTER}")
            print(image_count)
    ...
    def sweeping_square2(self, num_cells):
        with fits.open(f"{self.file_path}") as file:
            xs = np.array(file[1].data["X"])
            ys = np.array(file[1].data["Y"])
...
# test1 = Swift(
#     "/Users/hunterholland/Documents/Research/Laidlaw/Data/Modified/B59/Swift/xrt/event/sw00037645002xpcw3po_cl.evt.gz")
# test1.sweeping_square(10)

# test2 = Swift(
#     "/Users/hunterholland/Documents/Research/Laidlaw/Data/Modified/L1517/Swift/xrt/event/sw00034249004xpcw3po_cl.evt.gz")
# test2.sweeping_square(10)

# test3 = Swift(
#     "/Users/hunterholland/Documents/Research/Laidlaw/Data/Modified/L1517/Chandra/primary/acisf03755N004_evt2.fits.gz")
# test3.sweeping_square(10)

# test4 = Swift(
#     "/Users/hunterholland/Documents/Research/Laidlaw/Data/Modified/L1517/XMM/PPS/evt/P0101440801M1S001MIEVLI0000.FTZ")
# test4.sweeping_square(10)

test1 = Swift(
    "/Users/hunterholland/Documents/Research/Laidlaw/Data/Modified/B59/Swift/xrt/event/sw00037645002xpcw3po_cl.evt.gz")
test1.sweeping_square2(10)

test2 = Swift(
    "/Users/hunterholland/Documents/Research/Laidlaw/Data/Modified/L1517/Swift/xrt/event/sw00034249004xpcw3po_cl.evt.gz")
test2.sweeping_square2(10)

test3 = Swift(
    "/Users/hunterholland/Documents/Research/Laidlaw/Data/Modified/L1517/Chandra/primary/acisf03755N004_evt2.fits.gz")
test3.sweeping_square2(10)

test4 = Swift(
    "/Users/hunterholland/Documents/Research/Laidlaw/Data/Modified/L1517/XMM/PPS/evt/P0101440801M1S001MIEVLI0000.FTZ")
test4.sweeping_square2(10)
